...
@fibcint viser at vi ikke kan finne mer enn det 47. fibonacci-tallet uten å støte på en integer-overflow-error. Dette skyldes at svaret er større enn det som kan lagres i 32 bits(`int`).

#figure(image("../../imgs/week7-task5-2.png", width: 80%), caption: [C-kode som forsøker å finne fibonacci-tall 47 & 48]) <fibcint>

@fibclonglong viser den samme koden kjørt med `long long` istedenfor `int`. `main2.c` er tilpasset for `long long`. Her kan vi regne ut fibonacci-tall 93, men ikke 94.
